...
import { BlueTitle, GreyTitle, SectionHeading, SectionLabel } from "@/components/resuables";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { FEATURES, PLACEHOLDERS, STEPS, SUGGESTIONS } from "@/lib/data";
import { cn } from "@/lib/utils";
import { PricingTable, SignInButton, useAuth } from "@clerk/nextjs";
import { ArrowRight, ChevronRight, Link, Zap } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => { // it is to animate the placeholder text from data.ts file
  ...
  useEffect(() => { // it allows the textarea to grow as we write into it more
  ...
  const handleSubmit = () => { // it is to submit the prompt to the workspace page
    ...
    <main className="min-h-screen bg-[#0a0a0a] selection:bg-white/20">
        ...
        <div className="relative mx-auto mt-12 w-full max-w-2xl">
          <div className={cn(
            ...
            <textarea
              ...
              rows={1}
              className="w-full resize-none bg-transparent p-4 px-5 pt-5 text-sm
             focus:outline-none placeholder:text-white/20 sm:text-base"
              style={{ minHeight: 56, maxHeight: 200 }}

              placeholder={PLACEHOLDERS[placeholderIndex]}
            />

            <div className="flex item-center justify-between border-t border-white/6 px-4 py-2.5">
              <span className="text.xs text-white/20"> Press "ENTER" to generate & "Shift+Enter" for new line.</span>
          ...
          */}
            ...
            {SUGGESTIONS.map((s) => (
              <Button
                ...
                className="rounded-full border border-white/8 bg-white/4 px-3 py-1.5 text-xs text-white/40 hover:border-white/15 hover:text-white/70 hover:bg-white/8  "
              >
                {s}
              </Button>
            ))}
        ...
        <div className="mx-auto max-w-5xl overflow-hidden rounded-2xl border border-white/8 bg-[#0f0f0f] shadow-2xl shadow-black/60">
          <div className="flex items-center gap-2 border-b border-white/6 px-4 py-3">
            ...
            <div className="mx-auto flex h-6 w-64 items-center justify-center rounded-md bg-white/5 px-3">
              <span className="text-xs text-white/25">agentic-ai-builder/workspace</span>
            </div>
          </div>
      ...
      <section className="px-4 pb-32">
        <div className="mx-auto mb-14 max-w-5xl text-center">
          ...
          <SectionHeading grey="From promt" blue="to production." />
        </div>

        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-px overflow-hidden rounded-2xl border-white/6 bg-white/6 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <section className="px-4 pb-32">
        <div className="mx-auto mb-14 max-w-5xl text-center">
          ...
          <SectionHeading grey="Four steps" blue="to a working app." />
        </div>
            ...
            <div key={step.number} className="flex gap-6">
              <div className="flex flex-col items-center">
                <div className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/4">
                  <span className="font-mono text-xs font-semibold  text-white/50">{step.number}</span>
                </div>
                {/* line */}
                {i !== STEPS.length - 1 && (
                  <div className="h-full w-0.5 bg-white/4" />
                )}
              </div>
      ...
      <section className="relative mx-auto mb-32 max-w-5xl overflow-hidden rounded-2xl border border-white/8 px-10 py-24 text-center">
        ...
        <SectionHeading grey="Start building " blue="for free." />
        <p>
      ...
      <footer className="px-4 pb-12 text-center text-sm text-white/40">
        <p>Built with Gemini Flash 3.5 • Images generated by DALL·E 3 • Made by Arnav</p>
        <p className="mt-3 flex items-center justify-center gap-2">
          <Link
            onClick={() => window.open("https://github.com/negiarnav03", "_blank")}
